...
# Identify terminal nodes that violate the auxiliary-factor prior constraint.
get_invalid_leaf_ids_for_prior <- function(tree, common_vars, aux_factor_var) {
  terminal_nodes <- as.numeric(which(tree$tree_matrix[, "terminal"] == 1))
  if (nrow(tree$tree_matrix) == 1 || is.null(aux_factor_var)) return(numeric(0))
  
  terminal_ancestors <- get_ancestors(tree)
  if (is.null(terminal_ancestors) || nrow(terminal_ancestors) == 0) return(numeric(0))
  
  aux_table <- table(terminal_ancestors[, 1], terminal_ancestors[, 2])
  invalid <- NULL
  for (k in seq_len(nrow(aux_table))) {
    ancestor_vars <- names(aux_table[k, ])[aux_table[k, ] != 0]
    is_invalid <- any(vapply(aux_factor_var, function(z) all(ancestor_vars %in% z), logical(1)))
    if (is_invalid) invalid <- c(invalid, as.numeric(rownames(aux_table)[k]))
  }
  
  intersect(terminal_nodes, invalid)
}

# Return a logical indicator for whether each leaf is invalid.
leaf_invalid_flags <- function(tree, leaf_ids, common_vars, aux_factor_var) {
  invalid_ids <- get_invalid_leaf_ids_for_prior(tree, common_vars, aux_factor_var)
  leaf_ids %in% invalid_ids
}

# Extract the current leaf parameter values in the order of leaf_ids.
eval_mu_by_leaf <- function(tree, leaf_ids) {
  out <- numeric(length(leaf_ids))
  for (k in seq_along(leaf_ids)) {
    out[k] <- tree$tree_matrix[leaf_ids[k], "mu"]
  }
  out
}

# Build the Laplace proposal and evaluate the joint tree-leaf target kernel.
laplace_target_bundle <- function(tree, R, lambda, gamma2, sigma2, sigma2_mu,
                                  alpha, beta, common_vars, aux_factor_var,
                                  draw, eval_tree = NULL) {
  leaf_ids <- sort(unique(tree$node_indices))
...
# Joint tree-leaf update with a BART-style tree move and Laplace leaf proposal.
joint_update_tree_leaf_fast <- function(curr_tree, R, X, lambda, gamma2, sigma2, sigma2_mu,
                                        alpha, beta, node_min_size, s, iter, nburn,
                                        common_vars, aux_factor_var) {
  move_type <- sample_move(curr_tree, iter, nburn)
  
  # Propose a local tree-structure modification.
  ...
  # Evaluate the current state using the reverse leaf proposal density.
  old_bundle <- laplace_target_bundle(
    tree = curr_tree,
    R = R,
    lambda = lambda,
    gamma2 = gamma2,
  ...
  # Draw candidate leaf parameters and evaluate the proposed state.
  new_bundle <- laplace_target_bundle(
  ...
  # MH ratio with the joint target kernel and Laplace proposal correction.
  log_ratio <- new_bundle$log_target - old_bundle$log_target +
    old_bundle$log_q - new_bundle$log_q 
  
  accepted <- FALSE
  out_tree <- curr_tree
  if (!isTRUE(new_bundle$tree$ForceStump) &&
      is.finite(log_ratio) &&
      (log_ratio >= 0 || log(runif(1)) < log_ratio)) {
    out_tree <- new_bundle$tree
    accepted <- TRUE
  }
