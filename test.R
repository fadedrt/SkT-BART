...
prediction_skew <- function(x,y,test_x,test_y,npost,trees){
  y_mean = mean(y)
  y_sd = sd(y)
...
calculate_lpml_skewt <- function(y, x, npost=100, trees, sigma2_store, gamma2_store, v_store) {
  y_mean <- mean(y)
  y_sd <- sd(y)
  n <- length(y)
...
cat("sktBART的LPML为:"calculate_lpml_skewt(y=y,x=x,npost=2500,trees=sktbart_fit$trees,sigma2_store = sktbart_fit$sigma2,gamma2_store=sktbart_fit$gamma2,v_store = sktbart_fit$v))
